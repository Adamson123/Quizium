    ...
    >
      ...
      <div className="w-full h-[300px] mb-3">
        {!isLoading ? (
          <img
            src={
              quizInfo.coverImg ? bufferToObjUrl(quizInfo.coverImg) : quizImg2
            }
      ...
      <div className="px-3 flex flex-col items-center gap-3">
        ...
        <div className="flex w-full relative justify-center">
          ...
          {openQuizMenu && (
            ...
            >
              <p
                onClick={() => {
                  addToFavorite(quizInfo._id, inFavorites(quizInfo._id));
                }}
              ...
              >
                <span
                  className={`bi-star-fill ${
                    inFavorites(quizInfo._id) && "text-orange-500"
                  } `}
                ></span>
                <span className="text-[14px]">
                  {inFavorites(quizInfo._id) ? "Remove from" : "Add to"}{" "}
                  favorites
                </span>
              </p>
        ...
        {quizInfo?.description && (
          <p className="text-[12px] text-center">{quizInfo.description}</p>
        )}
        ...
        >
          {/* plays */}
          <p>
            <span className="bi-play-fill"></span> {quizInfo?.plays} plays{" "}
          </p>
          ...
          <p className="text-[13px] flex gap-1">
            <span className="bi-journal-text"></span> {getQuizType()?.type}
            {!isLoading && (
              <HoverForInfo
        ...
        <div className="flex gap-2">
          ...
          >
            {isHosting ? <Loading cus={"text-[14px]"} /> : "Host live"}
          </button>
        </div>
        {/* created by */}
        <div
        ...
        >
          ...
          <div className="flex flex-col text-center">
            ...
            <span className="text-[10px] text-grayFive">
              created on{" "}
              {quizInfo.createdAt && convtToSimpleDate(quizInfo?.createdAt)}
            </span>
          </div>
        </div>
      </div>
